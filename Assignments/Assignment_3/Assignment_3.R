...
iris[seq(2,150,2),] # create a subset of even-numbered rows
...
Sepal.Area <- iris$Sepal.Length * iris$Sepal.Width # multiply Sepal.Length and Sepal.Width and store in object called "Sepal.Area"
class(Sepal.Area)

# 4.  Add Sepal.Area to the iris data frame as a new column
iris$Sepal.Area <- Sepal.Area # add Sepal.Area to the data frame


# 5.  Create a new dataframe that is a subset of iris using only rows where Sepal.Area is greater than 20 
      # (name it big_area_iris)
area <- iris$Sepal.Area > 20
which(area == TRUE)
big_area_iris <- iris[area,]
big_area_iris
